...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="../pages/create.css">
...
<?php include 'includes/standardCollection.php' ?>
  ...
  <div id="content" class="p-4 p-md-5 pt-5">
    <h2 class="mb-4">Standard Collection via Filtering Payment Channels</h2>
...
if (curl_errno($curl)) {
    ...
    if ($arr === null) {
        ...
        // Map type values to their corresponding categories
        $typeMap = array(
            1 => "Online Bank",
        ...
        // Group processors by type and add them to categorized arrays
...
?>
  ...
  <fieldset style="width: 100%; height: flex; ">
    <legend>Payment Method</legend>

    <?php foreach ($categorizedProcessors as $type => $processors): ?>
        <?php
            // Exclude specific types from the dropdown
            $excludedTypes = array(32, 8, 1024, 4096, 64, 2048, 256);
            if (!in_array($type, $excludedTypes)):
        ?>
        <select class="custom-dropdown" id="ProcId_<?php echo $type; ?>" name="ProcId" >
            <option value="" selected><?php echo $typeMap[$type]; ?></option>
            <?php foreach ($processors as $processor): ?>
                <?php
                    $procId = $processor["procId"];
            $logoUrl = isset($processor["logo"]) ? $processor["logo"] : '';
            $longName = $processor["longName"];
            $serviceFee = '';

            $pattern = '/<b>(.*?)<\/b>/';
            $matches = [];
            if (preg_match($pattern, $processor["remarks"], $matches)) {
                $serviceFee = $matches[1];
            }

            $optionAttributes = 'data-img="' . $logoUrl . '" title="' . $serviceFee . '"';
            $option = '<option value="' . $procId . '" ' . $optionAttributes . ' >' . $longName . '</option>';
            echo $option;
...
<input type="text" name="ProcId" hidden>
<script>
