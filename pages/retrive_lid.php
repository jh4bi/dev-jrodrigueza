...
<fieldset>
        <h2 class="mb-4">Retrieve Lifetime ID</h2>
  <?php date_default_timezone_set('Asia/Manila'); $currentDate = date("l, F jS, Y"); echo $currentDate; ?>
     <form method="post">
        <label for="lifetime_id">Lifetime ID:</label>
        <input type="text" id="lifetime_id" name="lifetime_id" required maxlength="10">
        <input type="submit" name="submit" value="Retrieve Lifetime ID"></input>
    </form>
</fieldset>
    <?php
    if (isset($_POST['submit'])) {
        $lifetime_id = $_POST['lifetime_id'];

        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://test.dragonpay.ph/api/collect/v1/lifetimeid/$lifetime_id",
        ...
        if ($response === false) {
            ...
            echo '<input type="hidden" name="lifetime_id" value="' . $lifetime_id . '">';
  ...
  if (isset($_POST['deactivate'])) {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://test.dragonpay.ph/api/collect/v1/lifetimeid/deactivate/$lifetime_id",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        if ($deactivate_response === false) {
            ...
            echo '    text: "Lifetime ID ' . $lifetime_id . ' has been deactivated.",';
    ...
    if (isset($_POST['activate'])) {
     $lifetime_id = $_POST['lifetime_id'];
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://test.dragonpay.ph/api/collect/v1/lifetimeid/activate/$lifetime_id",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        if ($activate_response === false) {
            ...
            echo '    text: "Lifetime ID ' . $lifetime_id . ' has been activated.",';
            echo'     allowOutsideClick: false';
            echo '});';
            echo '</script>';
        }
